...
``non_tensor_batch['ot_e_obs']``).

The hook is disabled by default (alpha=0), so importing/calling it preserves
the stock trainer exactly.  When enabled, it adds a token-level shaping term
only on acquisition-action spans; observation and padding spans remain zero.
"""
...
def _record_signal(record: Mapping[str, Any], signal_key: str, allow_proxy: bool) -> float | None:
    ...
    for key in keys:
        if key == "proxy_e_obs" and not allow_proxy:
            continue
...
def apply_ot_opd_advantage(data: Any, config: Any = None) -> tuple[Any, dict[str, float]]:
    ...
    acquisition action, valid response-relative offsets, and (by default) a
    following action span.  This prevents crediting answer/observation tokens
    or stale offsets after response truncation.
    """
    ...
    for row in range(batch_size):
        ...
        external = _row_signal_map(_row_value(row_effects, row, batch_size))
        if settings["allow_proxy"]:
            external.update(_row_signal_map(_row_value(row_proxy_effects, row, batch_size)))
        for record_idx, record in enumerate(records):
            ...
            if end <= start:
                continue
            # Keep the credit on action tokens only.  The rollout loop marks
            # observations with response_mask=0; intersecting below provides a
            # second guard against malformed/shifted spans.
            candidates.append((row, start, end, float(value)))
            raw_values.append(float(value))

    if not candidates:
        return data, {"ot_opd/skipped_no_effect": 1.0}
    ...
    applied_tokens = 0
    applied_records = 0
    for (row, start, end, _), value in zip(candidates, values, strict=False):
