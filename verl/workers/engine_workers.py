...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_lora_adapter(self, local_path, global_step=0, base_model_path=None):
        """Export only the actor's trainable LoRA tensors for OT-OPSD.

        FSDP must gather the adapter on every rank before rank zero publishes
        it; callers therefore invoke this as a ONE_TO_ALL worker RPC.  The
        resulting directory is intentionally separate from the normal
        optimizer/model checkpoint and is published atomically with a
        ``READY`` marker.  No full 3B/7B model is materialised or written.

        The method is intentionally implemented on ``TrainingWorker`` rather
        than in the trainer process because the actor parameters live inside
        the FSDP engine.  Engines without a PEFT adapter fail closed instead
        of accidentally serialising a full model state dict.
        """
        ...
        except Exception as exc:  # pragma: no cover - dependency is runtime-only
            raise RuntimeError("save_lora_adapter requires safetensors") from exc

        model_cfg = self.model_config
        rank_value = int(getattr(self, "rank", 0))
        lora_rank = int(getattr(model_cfg, "lora_rank", 0) or 0)
        lora_adapter_path = getattr(model_cfg, "lora_adapter_path", None)
        if lora_rank <= 0 and not lora_adapter_path:
            raise RuntimeError("save_lora_adapter called but actor LoRA is disabled")

        # get_per_tensor_param performs the FSDP summon/gather and returns
        # adapter-only tensors when base_sync_done=True.  Materialise the
        # generator before entering the barrier so every rank participates in
        # identical collectives.
        params, peft_config = self.engine.get_per_tensor_param(
        ...
        for name, value in params.items():
            ...
            if key in adapter_state:
                raise RuntimeError(f"duplicate LoRA key after normalization: {key}")
            if hasattr(value, "full_tensor"):
                value = value.full_tensor()
            adapter_state[key] = value.detach().cpu().contiguous()
        if not adapter_state:
            raise RuntimeError("FSDP engine returned no LoRA tensors; refusing full-model fallback")
        ...
        config = jsonable(peft_config or {})
        ...
        config.setdefault(
            ...
            str(base_model_path or getattr(model_cfg, "local_path", None) or getattr(model_cfg, "path", "")),
        )

        target = Path(str(local_path)).expanduser().resolve()
        target.parent.mkdir(parents=True, exist_ok=True)
        tmp = None
        if rank_value == 0:
            if target.exists():
                raise FileExistsError(f"refusing to overwrite existing LoRA snapshot: {target}")
            tmp = Path(tempfile.mkdtemp(prefix=f".{target.name}.tmp-", dir=str(target.parent)))
            try:
                save_file(adapter_state, str(tmp / "adapter_model.safetensors"))
                (tmp / "adapter_config.json").write_text(
                    json.dumps(config, ensure_ascii=False, sort_keys=True, indent=2) + "\n", encoding="utf-8"
                )
                digest = hashlib.sha256()
                with (tmp / "adapter_model.safetensors").open("rb") as handle:
                    for chunk in iter(lambda: handle.read(1024 * 1024), b""):
                        digest.update(chunk)
                snapshot_meta = {
                    "kind": "otopsd_actor_lora",
                    ...
                    "sha256": digest.hexdigest()[:24],
                }
                (tmp / "otopd_snapshot.json").write_text(
                    json.dumps(snapshot_meta, ensure_ascii=False, sort_keys=True, indent=2) + "\n",
                    encoding="utf-8",
                )
                # READY is the publication barrier for readers (the scorer or
                # EMA helper).  It is deliberately written last.
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_lora_adapter(self, local_path, global_step=0, base_model_path=None):
        """Export a tiny actor LoRA snapshot for lagged OT-OPSD scoring."""

        assert "actor" in self.role, "save_lora_adapter only supports actor roles"
        return self.actor.save_lora_adapter(local_path, global_step, base_model_path)
